# NDE研究統計分析Python代碼
# NDE RESEARCH STATISTICAL ANALYSIS - PYTHON CODE
...
### 安裝必要的庫 / Install Required Libraries
...
```bash
python nde_statistical_analysis.py
```
...
1. **CSV文件** (Exported Data)
   - `personality_results.csv` - 人格維度結果
   - `death_anxiety_results.csv` - 死亡焦慮結果
   - `values_change_results.csv` - 價值觀改變結果

2. **圖像文件** (Visualizations)
   - `personality_comparison.png` - 人格維度比較圖
   - `death_anxiety_comparison.png` - 死亡焦慮比較圖
   - `effect_sizes.png` - 效應大小圖

3. **文本報告** (Reports)
   - `statistical_summary_report.txt` - 統計摘要報告
...
### 1. `calculate_cohens_d(group1, group2)`
計算兩組間的效應大小
```python
d = calculate_cohens_d(nde_data, control_data)
# 返回Cohen's d值 / Returns Cohen's d value
```

### 2. `compare_groups(nde_data, control_data, var_name)`
比較兩個獨立組
```python
result = compare_groups(nde_group, control_group, 'Agreeableness')
# 返回: 平均值、SD、t值、p值、Cohen's d / Returns: means, SDs, t, p, d
```

### 3. `paired_t_test(before, after, var_name)`
配對樣本t檢驗（NDE前後）
```python
result = paired_t_test(before_nde, after_nde, 'Universalism')
# 返回: 前後平均值、t值、p值、Cohen's d
```

### 4. `format_p_value(p_val)`
將p值格式化為可讀形式
```python
formatted = format_p_value(0.0005)
# 返回: '<.001'
```
...
```python
# 替換這些行 / Replace these lines:
personality_data = {
    'Agreeableness_NDE': your_actual_nde_agreeableness_data,
    'Agreeableness_Control': your_actual_control_agreeableness_data,
    # ... 其他維度 / ... other dimensions
}
```
...
**此代碼已準備好直接使用！**
**This code is ready to use directly!**
